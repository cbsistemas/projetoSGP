...
begin
  ...
  begin
    ...
    Finally
     ...
     begin
       frmAberturaCaixa.Free;
        Case MessageBox(Application.Handle, Pchar('Deseja imprimir a Abertura?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
        idYes:
          begin
            //ImprimirComprovante;
  ...
  begin
    Case MessageBox(Application.Handle, Pchar('Caixa ainda em aberto, deseja fechar o Caixa?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    idYes: begin
              statusCaixa := 'LV';
              MovimentoCaixa;
           end;
...
begin
  ...
  if idFormaPag = 4 then
    campo := 'MC.VLR_CHEQUE';
 //Não sei pq mas esta acertando o movimento da venda q esta sendo devolvida
 with qryComand do
  begin
    Close;
    SQL.Clear;
    ...
    ParamByName('VLRDEVOLVER').AsFloat := vlrDevolucao;//- vlrEntrada;   //parei ak na questão do valor de entrada
    Prepare;
    ExecQuery;
  end;
  transFrmPrincipal.CommitRetaining;
...
begin
   ...
   begin
    Application.MessageBox('Desculpe, Nenhum valor unitário  informado! ', 'Valor Inválido.', MB_OK+MB_ICONERROR);
    edtItemQtn.SetFocus;
    Abort;
   end;
  ...
  begin
    ...
    begin
      ...
      Application.MessageBox('Desculpe, formato ´digitado do Preço Unitário está inválido.', 'Formato Inválido.', MB_OK+MB_ICONERROR);
      Abort;
    end;
    ...
    begin
      ...
      begin
        TFrmMensagens.Mensagem('Desculpe,Usuário sem permissão para alterar o preço de venda.', 'Sem permissão.','I',[mbOK]);
        fdsVendaItensVLR_UNITARIO.AsCurrency :=  itemVlrUnitario;
        Abort;
      end;
    end;
  ...
  begin
    ...
    Finally
      ...
      begin
        ...
        redTempItens.Lines.Add('');
        redTempItens.Lines.Add(' ' + 'Item Nº ' + frmCancelarItem.fdsVendaItensITEM.AsString + ' - CANCELADO');
        redTempItens.Lines.Add('');
        ...
        redItens.Lines.Add('');
        redItens.Lines.Add(' ' + 'Item Nº ' + frmCancelarItem.fdsVendaItensITEM.AsString + ' - CANCELADO');
        redItens.Lines.Add('');
...
begin
    ...
    try
      ...
      if not(OpenComm('COM3', '9600,n,8,2')) then
        Application.MessageBox('Não foi possível conectar com a impressora!', 'Atenção', MB_OK+MB_ICONERROR);
...
begin
  ...
  begin
    Try
     ...
     Application.MessageBox('Venda em andamento, finalize ou termine primeiro.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
...
begin
  ...
  //   ProxyPass := edtProxySenha.Text;
    ...
    with ACBrNFe1.Configuracoes.WebServices do
    begin
      AguardarConsultaRet := 100;       //<----- Tempo que o componente irá aguardar pelo retorno da consulta ao protocolo de recebimento
      ...
      IntervaloTentativas := 1000;       //<------ intervalo que o componente irá aguardar antes de tentar novamente

      Tentativas := 5;                   // <------ numero de tentativas que o componente irá fazer antes de emitir o erro
    end;
  ...
  begin
    try
      ...
      begin
        ...
        begin
          ConsultaItem;
         case MessageBox(Application.Handle,pchar('Deseja imprimir a Venda?'),'Atenção',MB_YESNO +MB_ICONQUESTION + MB_DEFBUTTON2)of
           IDYES: begin
                    imprimir:=True;
                  end;
            IDNO: begin
        ...
        begin
          ...
          begin
            ...
            begin
              ExibeNumeroNota.Interval := 2000; // Tempo que a msg ficará na tela -> 2 Segs
...
begin
  ...
  begin
    TFrmMensagens.Mensagem('Seu sistema expirou, entre em contato o desenvolvedor.', 'Atençao.', 'X',[mbOK]);
  ...
  begin
   ...
   // vlrTotal := vlrTotal + ((itemVlrUnitario) * fdsVendaItensQTN.AsFloat);
    ...
    begin
      ...
      lblVlrCredito.Caption := 'Crédito: ' + FormatFloat('R$ #,##0.00', vlrDevRestante);
    end;
    ...
    begin
        ...
        begin
          ...
          if qryVerificaPagReceber.RecordCount>0 then
            ...
            begin
              ...
              haContaPaga:=True;
              Application.MessageBox('Devido haver pagamento parcial ou total da venda, só haverá a possibilidade de fazer lançamento de crédito na Conta do Cliente.', 'Devolução', MB_OK+MB_ICONEXCLAMATION);
            end
            else
            begin
      ...
      Finally
        ...
        begin
          ...
          begin
            lblVlrCredito.Visible := True;
            lblVlrCredito.Caption := 'Crédito: ' + FormatFloat('R$ #,##0.00', vlrDevolucao);
          end
          else
          begin
            fdsMovimentoCaixa.Open;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  fdsMovimentoCaixaHISTORICO.AsString := 'Saída em Dinheiro no valor de ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' referente a devoloução da venda nº ' + IntToStr(idVendaDevolvida);
                  fdsMovimentoCaixa.Post;
                  historico:='Abatimento no valor de ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' referente a devoloução da venda nº ' + IntToStr(idVendaDevolvida);
                end
                else
                begin
                  fdsMovimentoCaixaVLR_VENDA.AsFloat := vlrDevolucao;
                  fdsMovimentoCaixaTROCO.AsFloat := vlrEntrada;
                  fdsMovimentoCaixaHISTORICO.AsString := 'Devolução da venda nº ' + IntToStr(idVendaDevolvida) + '. Excluido Nota no Contas a Receber.';
                  fdsMovimentoCaixa.Post;
                ...
                Application.MessageBox('Devolução de venda realizada com sucesso.', 'Devolução', MB_OK+MB_ICONEXCLAMATION);
              end
              else
              begin
                if (tempIDFormaPag = 1) or (tempIDFormaPag = 9) then
                begin
                  fdsMovimentoCaixaVLR_VENDA.AsFloat := vlrDevolucao;
                  fdsMovimentoCaixaTROCO.AsFloat := vlrDevolucao;
                  fdsMovimentoCaixaHISTORICO.AsString := 'Saída em Dinheiro no valor de ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' referente a devoloução de itens da venda nº ' + IntToStr(idVendaDevolvida);
                  fdsMovimentoCaixa.Post;
                  historico:='Abatimento no valor de ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' referente a devoloução de itens da venda nº ' + IntToStr(idVendaDevolvida);
                end
                else
                begin
                  fdsMovimentoCaixaVLR_VENDA.AsFloat := 0;
                  fdsMovimentoCaixaHISTORICO.AsString := 'Recalculado Conta do Cliente devido a devolução de itens da venda nº ' + IntToStr(idVendaDevolvida) + '  ' + FormatFloat('R$ #,##0.00', vlrDevolucao);
                  fdsMovimentoCaixaTROCO.AsFloat := ExcluirParteReceber(idVendaDevolvida, vlrDevolucao);
                  fdsMovimentoCaixa.Post;
                  historico:= 'Recalculado Conta do Cliente devido a devolução de itens no valor de '+FormatFloat('R$ #,##0.00', vlrDevolucao)+' da venda nº ' + IntToStr(idVendaDevolvida) + ' .';
                  AcertarVlrMoviNotaDevolvida(idVendaDevolvida, tempIDFormaPag);
                end;

                Application.MessageBox('Devolução realizada com sucesso.', 'Devolução', MB_OK+MB_ICONEXCLAMATION);
              end;
              Case MessageBox(Application.Handle, Pchar('Deseja Imprimir o comprovante de devolução?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
                     IDYES: begin
                               with qryComand do
                               begin
                                Close;
            ...
            begin
              ...
              fdsMovimentoCaixaHISTORICO.AsString := 'Crédito no valor de ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' da devolução de itens da venda nº ' + IntToStr(idVendaDevolvida);
              fdsMovimentoCaixa.Post;
            end;
            transFrmPrincipal.CommitRetaining;
          end;
          ...
          begin
            ...
            begin
              ...
              if frmParamDevTroca.tipo = 'credito' then
              begin
                Case MessageBox(Application.Handle, Pchar('Deseja Imprimir o comprovante de lançamento de crédio para esse cliente?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
                     IDYES: begin
                               with qryComand do
                               begin
                                Close;
                              ...
                              ImprimirComprovanteDev('/Relatorios/frxComprovanteCredito.fr3');
                               ...
                               begin
                                obs:='Credito lançado na conta do Cliente devido haver pagamento parcial ou total de parcelas da venda.';
                                DM.obs:= obs;
                                ImprimirComprovanteDev('/Relatorios/frxComprovanteCredito.fr3');
                               end;
                            end;
              ...
              Application.MessageBox('Crédito lançado com sucesso.', 'Devolução', MB_OK+MB_ICONEXCLAMATION);
            end;
          end;
        end;
...
//
    ...
    begin
      ...
      begin
       ...
       begin
        Application.MessageBox('Informe uma quantidade', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
        edtItemQtn.Text:= '1';
       ...
       // ShowMessage(IntToStr(fdsVendaItens.RecordCount) + ' ' + fdsVendaItensNOME.AsString);
        ...
        begin
          ...
          begin
            ...
            fdsVendaIDCAIXA.AsInteger := DM.numCaixa; // Tomar cuidado pois está puchando o número do caixa no ini e passando como ID do caixa para a venda
            Post;
          end;
          transFrmPrincipal.CommitRetaining;
...
//           Case MessageBox(Application.Handle, Pchar('Deseja realmente fechar o sistema?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    Case TFrmMensagens.Mensagem('Deseja realmente fechar o sistema?','Atenção','S',[mbNao,mbSim])of
         true: begin
            with qryComand do
            begin
              Close;
            ...
            if qryComand.RecordCount > 0 then
              //Application.MessageBox('Lembrando que você possui Cupons Fiscais emitidos em modo OFF-Line, que devem ser transmitidos em menos de 24h.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
             TFrmMensagens.Mensagem('Lembrando que você possui Cupons Fiscais emitidos em modo OFF-Line, que devem ser transmitidos em menos de 24h.','Atenção','C',[mbOk]);
            Application.Terminate;
         end;
    end;
 end
...
//            else
      AbandonaVenda;
            ...
            //Application.MessageBox('Você não pode fechar o sistema com uma venda em andamento, finalize ou termine primeiro.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
end;
...
begin
  ...
  begin
    if fdsVendaItens.State in [dsInsert, dsEdit] then
    begin
      Application.MessageBox('Você não terminou de lançar o item atual, cancele ou confirme este item antes de fechar a venda.', 'Atenção', MB_OK+MB_ICONWARNING);
    end
    else
    begin
      Try
      ...
      Finally
        ...
        begin
          ...
          //Valor da venda continua o mesmo, somente com os descontos nos itens, o valor total é o valor da venda - desconto no total
          ...
          if vlrDevolucao > 0 then
          begin
            fdsMovimentoCaixaHISTORICO.AsString := 'Abatido ' + FormatFloat('R$ #,##0.00', vlrDevolucao) + ' de Crédito';
            fdsMovimentoCaixaHISTORICO.AsString := fdsMovimentoCaixaHISTORICO.AsString + ' da devoloução de itens da venda nº ' + IntToStr(idVendaDevolvida);
          end
          else
          begin
            if frmVendaFinalizacao.valorCredito > 0 then
            begin
              fdsMovimentoCaixaHISTORICO.AsString := 'Abatido ' + FormatFloat('R$ #,##0.00', frmVendaFinalizacao.valorCredito) + ' de Crédito do cliente';
            end;
          end;
          ...
          begin
           ...
           lblTabelaPreco.Caption:='Tab.Preço '+IntToStr(DM.fdsConfigTAB_PADRAO_PDV.AsInteger);
           DM.fdsCaixa.Post;
           dm.conexao.CommitRetaining;
          end;
        end;
...
//  if (Date > StrToDate('30/08/2015')) or (dm.fdsConfigFABRICA.AsString <> 'N') then //alterar o nome do campo e utilizar para quando o sistema explirar alterar essa letra para um J, qualquer uma diferente dessa expira, juntar com lógica de CNPJ gavado no config
//  begin   //criar logica para toda vez que abrir, armazenar a data atual, se abri denovo com data menor, o sistema informará data incorreta. Armazenar a data atual criptografada no banco para comparação.
//    with DM do
//    begin
//      fdsConfig.Edit;
...
//    end;
//    Application.MessageBox('Seu sistema expirou, por favor entre em contato com Vítor Ferreira Rodrigues.', 'Atenção', MB_OK+MB_ICONWARNING);
//    Application.Terminate;
//  end;
...
begin
  ...
  begin
   ...
   begin
    TFrmMensagens.Mensagem('Desculpe, Usuário sem permissão para abrir a Gaveta.', 'Não permitido abrir gaveta.', 'I',[mbOK]);
      Abort;
   end;
  ...
  begin
   ...
   begin
      TFrmMensagens.Mensagem('Desculpe, Usuário sem permissão para abrir Gaveta.', 'Não permitido abrir gaveta.','I' ,[mbOK]);
      Abort;
   end;
 ...
 { if (Key = VK_F5) and (modoEdicaoTela = False) then
  ...
  begin
    ...
    begin
       ...
       Application.MessageBox('Nenhum Modelo Especificado!', 'Atenção',MB_OK+MB_ICONQUESTION)
    end;
  end
  else
  begin
  ...
  begin
    modoEdicaoTela := True;
    lblProduto.Caption := 'Sistema em Modo de edição';
...
//    begin
//      Application.MessageBox('Por favor,primeiro termine de lançar o item', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
//      fdsVendaItens.Last;
//      abort;
//    end;
end;
...
begin
//  Use Printers
//  Printer.PrinterIndex := -1; Seleciona impressora default, para evitar erro após muitas impressoes
...
Var TimeOut : Integer ;
begin
    //showMessage('Produto de Balança');  //lembrar de comenta
     try
    ...
    // showMessage('pegou o peso');
     ...
     begin
      if (FloatToStr(ACBrBAL1.LePeso) ='-9') or ( FloatToStr(ACBrBAL1.LePeso) ='-9,000') then
       begin
        showMessage('Erro na comunicação com a balança!Por favor, verifique o se a balança esta conectada ao computador ou se o protocolo de comunicação esta de acordo com as configurações');
        edtItemQtn.Text := FloatToStr(ACBrBAL1.LePeso);
       end;
     end;
   // showMessage('Não usa Balança');
   Except on E : Exception do
     begin
      ShowMessage('Erro na leitura da Balança: ' + E.Message);
     end;
...
begin
  ...
  begin
    ...
    if DM.fdsUsuarioPermicFECHAMENTO.AsString = 'N' then
     begin
       // Application.MessageBox('Desculpe, você não tem permição para entrar em Movimentação', 'Ver Movimentação não Permitido.', MB_OK+MB_ICONERROR);
        TFrmMensagens.Mensagem('Desculpe, Usuário sem permissão para fechar o caixa.', 'Sem permissão.', 'I',[mbOK]);
        try
         frmLogin := TfrmLogin.Create(nil);
         frmLogin.ShowModal;
        finally
    ...
    Application.MessageBox('Finalize ou Cancele a venda atual primeiro!', 'Atençaõ', MB_OK+MB_ICONEXCLAMATION);
end;
...
//          modelo := 55;
        ...
        try
          ...
          frmStatusNFCe.lblRetorno.Caption := 'Aguarde inutilizando numeração';
          frmStatusNFCe.ShowModal;
        finally
          if (frmStatusNFCe.AutorizaDeletar = true) and ( frmStatusNFCe.StatusCancelada <> 'DV')then
          begin
    ...
    Finally
      ...
      begin
        ...
        begin
          ...
          fdsVendaVLR_TOTAL.AsFloat := frmSelPreVenda.fdsPreVendaVLR_VENDA.AsFloat;  //Foi usado assim para resolver o problema do disconto do pré-venda
...
begin
  ...
  begin
    ...
    begin
      ...
      Application.MessageBox('Você não pode fazer um recebimento com uma venda em andamento, finalize ou termine primeiro.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
  end
  else
  begin
    TFrmMensagens.Mensagem('Desculpe, Usuário sem permissão para realizar recebimentos.', 'Não permitido Receber.','I' ,[mbOK]);
    Abort;
  end;
end;
...
begin
  ...
  begin
    // Pesquisa o Produto pelo código digitado
    with fdsItens do
    begin
      Close;
...
begin
 ...
 begin
    TfrmMensagens.Mensagem('O certificado digital '+ frmPrincipal.ACBrNFe1.SSL.CertRazaoSocial+' instalado que está prestes a vencer,vencerá em '+
 ...
 begin
   ...
   begin
     TfrmMensagens.Mensagem('O certificado digital '+ frmPrincipal.ACBrNFe1.SSL.CertRazaoSocial+' utilizado para emissão dos documentos eletônicos esta VENCIDO!'
...
begin
  ...
  begin
    ...
    begin
      ...
      TFrmMensagens.Mensagem('Desculpe, usuário sem permissão para ultrapassar o desconto máximo.', 'Desconto não permitido.','I',[mbOk]);
      Abort;
    end;
  end;
end;
...
begin
  ...
  begin
    TFrmMensagens.Mensagem('Desculpe, você não tem permissão para realizar esta operação', 'Sem permissão.','I',[mbOK]);
    Result:= False;
  end;
end;
...
//        if fdsCntReceber_Item.RecordCount > 0 then
//        begin
//          Case MessageBox(Application.Handle, Pchar('Deseja imprimir a via do cliente?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
//            idYes: begin
//                      ImprimeCupomFastReport;
//                   end;
//          end;
