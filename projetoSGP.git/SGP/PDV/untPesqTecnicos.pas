...
begin
  ...
  sql := 'SELECT F.* FROM FUNCIONARIO F WHERE F.STATUS <> ' + QuotedStr('I');
...
begin
  fdsTecnicos.Close;
  fdsTecnicos.SelectSQL.Clear;
  sql := 'SELECT F.* FROM FUNCIONARIO F WHERE UPPER(F.NOME) LIKE '+ QuotedStr(edtNome.Text + '%') + ' AND F.STATUS <> ' + QuotedStr('I');
  fdsTecnicos.SelectSQL.Add(sql);
  fdsTecnicos.OrderClause := 'NOME';
  fdsTecnicos.Prepare;
  fdsTecnicos.Open;
