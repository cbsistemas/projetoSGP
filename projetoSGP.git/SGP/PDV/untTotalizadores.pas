...
type
  ...
  public
    var
      ...
      NTVenda, FNVenda, TKVenda, VDVenda ,VReceb: Double;
...
var
  ...
  tempSangriaDN, tempSuprimentoDN, TempSangriaCH,tempTotalEN,tempTotalReceb : Double;
...
begin
  try
    ...
    begin
      ...
      ExecQuery;
      if not(Trim(FieldByName('TOT').AsString) = EmptyStr) then
        tempTotalReceb := RoundTo(FieldByName('TOT').AsFloat, -2)
      else
        tempTotalReceb := 0.00;
      ...
      Close;
      SQL.Clear;
      SQL.Add('SELECT SUM(TROCO) AS TOT FROM MOVIMENTO_CAIXA WHERE TIPO = ' + QuotedStr('DV') + ' AND CONTROLE = ' + DM.fdsCaixaCONTROLE.AsString);
      SQL.Add(' AND IDCAIXA = ' + IntToStr(DM.numCaixa) + ' AND IDFILIAL = ' + IntToStr(DM.fdsConfigIDFILIAL.AsInteger));
      Prepare;
      ExecQuery;
      if not(Trim(FieldByName('TOT').AsString) = EmptyStr) then
