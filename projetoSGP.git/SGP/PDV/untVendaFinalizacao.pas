...
begin
 ...
 // vendaEntregar := True;//vitor
  ...
  begin
    ...
    lblVlrCredito.Caption := 'Crédito: ' + FormatFloat('R$ #,##0.00', valorCredito);
...
//  //Vitor está parte foi feita para não ter que alterar a forma como calcula o valor inicial a pagar
...
begin
  ...
  begin
    ...
    //begin
      ...
      begin
        TFrmMensagens.Mensagem('O limite de crediário para esse cliente é de R$'+FloatToStr(fdsClienteLIMITE_CREDITO.AsFloat), 'Cliente Bloqueado','L',[MBOK]);

        temPermicao := uVerificaPermicao.Verifica('LIBERAR_CLIENTE_BLOQ');

        if not(temPermicao) then
        begin
          TFrmMensagens.Mensagem('Desculpe,usuário sem permissão para ultrapassar o limite de crediário de cliente.', 'Crediário não permitido.','I',[mbOk]);
          abort;
        end;
      end;
    //end;
...
begin
  ...
  begin
    Application.MessageBox('Total da Venda não atingido.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
    edtDinheiro.SetFocus;
    Abort;
  end;
 ...
 // begin
    ...
    begin
      ...
      if idCliente = DM.fdsConfigIDCLIENTEPADRAO.AsInteger then
      begin
        Application.MessageBox('Informe o Cliente para a entrega.', 'Atenção', MB_OK+MB_ICONEXCLAMATION);
...
begin
  ...
  begin
     ...
     begin
       ...
       begin
          Application.MessageBox(Pchar('Desconto máximo permitido na venda é: ' + FloatToStr(descMaxVenda) + '%'), 'Desconto não permitido.', MB_OK+MB_ICONERROR);
...
// ShowMessage(gdrNota.SelectedField.AsString);  Lê o conteúdo da célula celecionada
end.
