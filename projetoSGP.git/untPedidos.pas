...
var
  ...
  // usadas para calculos no relatório de Pedidos total agrupado por cliente
...
begin
  ...
  begin
    mensagem:='Faturamento não permitido!'#13'Não sera possível realizar o faturamento do pedido '+fdsPedidoID.AsString+' pois o mesmo está vínculado a uma NFe que já foi transmitida a SEFAZ.';
    Application.MessageBox(pWideChar(mensagem),'Operação Anulada', MB_ICONEXCLAMATION + MB_OK);
    //TfrmMensagemPermissao.Mensagem(mensagem,'Operação Anulada','E',[mbOK]);
    abort;
  end;
end;
...
begin
  ...
  begin
    Case MessageBox(Application.Handle,
      Pchar('Deseja realmente Confirmar esse Pedido?'), 'Atenção',
...
begin
  ...
  begin
    ...
    begin
    mensagem:='Erro ao Excluir o pedido : '+fdsPedidoID.AsString+#13'Esse pedido não pode ser excluido pois está vínculado a uma NFe.'#13'Caso a NFe esteja em aberto é possível fazer a exclusão da mesma e excluir o pedido desejado,em outros casos não será possível realizar a operação.';
      Application.MessageBox(pWideChar(mensagem),'Operação Anulada', MB_ICONEXCLAMATION + MB_OK);
      abort;
    end;
  end;
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir o pedido ou Orçamento atual? Não será possivel desfazer a operação.'),
    ...
    idYes:
      begin
        try
          ...
          Application.MessageBox('Pedido ou Orçamento excluído com sucesso!',
          ...
          Application.MessageBox
            ('Erro ao excluir Pedido ou Orçamento, operação anulada.', 'Erro',
            MB_ICONERROR + MB_OK);
          DM.Conexao.RollbackRetaining;
...
begin
  ...
  // else
  // Application.MessageBox
  // ('Este Pedido já foi faturado, consulte em Contas a Receber.', 'Atenção',
...
begin
  ...
  begin
    Case MessageBox(Application.Handle,
      Pchar('Deseja realmente Fechar esse Pedido?'), 'Atenção',
      MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
      idYes:
        begin
          fdsPedido.Edit;
          ...
          GravaCancela('CF');
          // A idéia é que a tela continue nos pedidos a serem confirmados
          qrySql.Close;
        end;
    end;
  end;
...
begin
  ...
  begin
    Application.MessageBox('Informe um vendedor.', 'Anteção',
      MB_OK + MB_ICONERROR);
    abort;
  end;
  if fdsPedidoIDCLIENTE.AsInteger = 0 then
  begin
    Application.MessageBox('Informe um Cliente.', 'Anteção',
      MB_OK + MB_ICONERROR);
    abort;
  end;
  ...
  begin
    ...
    begin
      Application.MessageBox
        ('Você está incluindo ou alterando um item, grave a inclusão/alteração deste primeiro e depois grave o pedido ou o Orçamento.',
        'Anteção', MB_OK + MB_ICONERROR);
      edtQtnItem.SetFocus;
      abort;
    end;
    ...
    begin
      Application.MessageBox
        ('Você não pode gravar um Pedido ou um Orçamento sem itens vendidos.',
        'Anteção', MB_OK + MB_ICONERROR);
    ...
    begin
      Case MessageBox(Application.Handle, Pchar('Deseja Excluir o pedido ou orçamento duplicados?'),
      'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
        idYes:
        begin
          if VerificaFaturamento(idPedidoExcl) = True then
          begin
            mensagem:='Erro ao Excluir o Pedido/Orçamento : '+fdsPedidoID.AsString+#13'Esse Pedido/Orçamento não pode ser excluido pois está vínculado a uma NFe.'#13'Caso a NFe esteja em aberto é possível fazer a exclusão da mesma e excluir o pedido/Orçamento desejado,em outros casos não será possível realizar a operação.';
            Application.MessageBox(pWideChar(mensagem),'Operação Anulada.Pedido/Orçamento não Excluido.', MB_ICONEXCLAMATION + MB_OK);
          end
          else
          begin
    ...
    Begin
      Case MessageBox(Application.Handle, Pchar('Deseja imprimir Orçamento?'),
        'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
        idYes:
          begin
...
begin
  ...
  // cartaoDB:= 'cartão de débito, ';
  // ShowMessage('Não foi possível completar a ação!Pois o pagamento desse pedido foi efetuado em '+dinheiro+cartaoDB+'não possuindo parcelas.');
  // Abort;
  // end;
end;
...
//        fdsNFeVLR_TOTAL.AsFloat := fdsPedidoVLR_FRETE.AsFloat -fdsPedidoVLR_DESCONTO.AsFloat;
    ...
    begin
      mensagem:='Devido ao lançamento do desconto no valor de R$'+fdsPedidoVLR_DESCONTO.AsString+' na finalização do pedido PD'+fdsPedidoID.AsString+' ,será feito o rateamento desse valor nos itens desse pedido.';
      Application.MessageBox(pWideChar(mensagem),'Atenção', MB_ICONINFORMATION + MB_OK);
    end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Salve ou Cancele a Inclusão/Alteração do pedido antes de sair.',
...
begin
  ...
  begin
    Application.MessageBox('Primeiro informe um período de vendas.', 'Atenção',
...
begin
  ...
  begin
    ...
    if fdsPedidoPREFIXO.AsString = 'PD' then
      ...
      fdsPedido.Next;
    // prior indica para voltar ao anterior e next para proseguir para o próximo
  end;
  ...
  begin
    ...
    if cdsItensPedido.RecordCount <= 29 then
      // Ao todo cabem 25 itens na primeira página de um pedido
...
begin
  ...
  begin
    ...
    Application.MessageBox('Não existe venda no período informado.', 'Atenção',
  ...
  Finally
    ...
    begin
      ...
      try
        ...
        frmStatusNFe.lblRetorno.Caption := 'Aguarde inutilizando numeração';
        frmStatusNFe.ShowModal;
      finally
        frmStatusNFe.free;
      end;
...
begin
  ...
  begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  TfrmMenssagens.Mensagem('Desculpe,Usuário sem permissão para vender produtos com estoque zerado.', 'Sem permissão.','I',[mbOK]);
                  Abort;
                end;
              end
              else
    ...
    begin
      ...
      cdsItensPedidoVLR_UNITARIO.AsFloat  := qrySql.FieldByName('PREC_VENDA').AsFloat; // vitor ver sobre tabela de preço
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Append;
        cdsFormaPagMOEDA.AsString := 'Cartão: ';
