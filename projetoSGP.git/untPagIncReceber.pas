...
type
  ...
  var
    modo: string[10];
    // tipo = receber ou pagar / modo = inclusão ou pagamento serve para os 2 tipos
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar o Recebimento?'), 'Atenção',
...
begin
  ...
  if (modo = 'Inclusão') or (modo ='Renegociar') then
  begin
    if valorNota > 0 then
    begin
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Primeiro gere as parcelas a receber.', 'Atenção',
  ...
  if (modo = 'Pagamento') then
    Application.MessageBox('Documento Baixado Com Sucesso!',
      'Confirmação de Baixa', MB_OK + MB_ICONEXCLAMATION);
...
begin
  ...
  begin
    Application.MessageBox
      ('Crédito informado é maior que o crédito em conta do cliente.',
...
begin
  ...
  begin
    ...
    if (valorDoc <= 0) then
    begin
      Application.MessageBox('Informe um valor para o Documento.', 'Atenção',
        MB_OK + MB_ICONEXCLAMATION);
...
begin
  ...
  if (modo = 'Inclusão') then
  begin
    frmPagIncReceber.Caption := 'Inclusão de Nota de Cliente';
  ...
  if (modo = 'Renegociar') then
  begin
   frmPagIncReceber.Caption := 'Renegocição de Nota(s) de Cliente';
...
begin
    ...
    begin
      ...
      begin
        TfrmMenssagens.Mensagem('Desculpe, Usuário não possui permissão para não usar o juros aplicado as parcelas.', 'Sem permissão.','I',[mbOK]);
