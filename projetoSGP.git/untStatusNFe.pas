...
begin
  try
    lblRetorno.Caption := 'Aguarde Cancelando Manifesto Eletrônico ...';
  ...
  Except
    ...
    begin
      ...
      mmoErro.Lines.Add
        ('Ocorreu o seguinte erro ao cancelar o Manifesto Eletrônico:');
      mmoErro.Lines.Add('Código do Erro: ' +
        IntToStr(frmMDFe.ACBrMDFe1.WebServices.EnvEvento.EventoRetorno.
        retEvento.Items[0].RetInfEvento.cStat) + '   NFe Nº: ' +
    ...
    begin
      ...
      begin
        ...
        mmoErro.Lines.Add('Código do Erro: ' +
          ...
          '   NFe Nº: ' + cdsNFeCODIGO_NFE.AsString);
...
begin
  frmPrincipal.ACBrNFe1.EventoNFe.Evento.Clear;
  sequencia := '1';
  InputQuery('Informe o número de seguência desta Carta:', 'Sequência da Carta',
    sequencia);
  ...
  begin
    ...
    begin
      ...
      begin
        // retornos
        lblRetorno.Caption := 'Carta de Correção enviada com sucesso';
        btnOK.Enabled := True;
        mmoErro.Lines.Add(EventoRetorno.retEvento.Items[0]
          .RetInfEvento.xMotivo);
  ...
  begin
    ...
    begin
      lblRetorno.Caption := 'Ocorreu algum erro';
      mmoErro.Lines.Add
        ('Ocorreram erros ao tentar enviar a Carta de Correção:');
      mmoErro.Lines.Add('Lote: ' + IntToStr(EventoRetorno.idLote) +
        '    Ambiente: ' + TpAmbToStr(EventoRetorno.tpAmb));
      mmoErro.Lines.Add('Orgao: ' + IntToStr(EventoRetorno.cOrgao) +
        '    Status: ' + IntToStr(EventoRetorno.cStat));
...
Var
  ...
  NFRef: TNFrefCollectionItem; // Adiciona as notas fiscais que serão devolvidas
...
begin
  try
    ...
    NotaF.NFe.Ide.tpAmb := frmPrincipal.ACBrNFe1.Configuracoes.WebServices.
      Ambiente; // Lembre-se de trocar esta variável quando for para ambiente de produção
    ...
    // Verifica se é devolução e faz com que em anoações adicionas da nota apareça o valores informados em BICMS ST e VICMS ST
    ...
    // Dados do responsável Técnico    (EM breve será solicitado)
    ...
    // Início de informações do Cliente
    ...
    begin
      ...
      // informa se o item entrá no calculo total da nota
      ...
      begin
        ...
        // Explicação pg 4 do Manual de Preenchimento_NFC-e
      end
      else
      begin
      ...
      Produto.Prod.xProd := StringReplace(cdsItensNFePRODUTO.AsString, 'Ç', 'C',
        [rfReplaceAll]);
      ...
      // Aqui eu somo o desconto aplicado no total rateado para os itens com o desconto unitário aplicado direto nos itens durante a venda
      vlrTotDesc := vlrTotDesc + Produto.Prod.vDesc;
      // Soma o total de desconto aplicado para informar no cabeçalho da nota
      ...
      // Produto.Imposto.ICMS.vBC     := 100;   //Vítor Fazer o cálculo dos impostos
    ...
    // NotaF.NFe.Total.ICMSTot.vBC     := 100;  //Vítor Fazer o cálculo dos impostos
    ...
    // Não foi usado o campo VLR_DESCONTO da tabela VENDA pois esse campo só armazena o desconto dado direto no total da nota
    ...
    if (copy(NotaF.NFe.Ide.natOp, 1, 9) = 'DEVOLUÇÃO') or
      (copy(NotaF.NFe.Ide.natOp, 1, 9) = 'Devolução') or
      (copy(NotaF.NFe.Ide.natOp, 1, 9) = 'devolução') or
    ...
    begin
      ...
      begin
        NotaF.NFe.InfAdic.infAdFisco := NotaF.NFe.InfAdic.infAdFisco +
          'NFA-e emitida por MEI. Caso o destinatário seja contribuinte do ICMS, é obrigatória a emissão de NF-e de entrada (art. 35, § 5º, do Anexo I do Livro VI do RICMS). ';
        NotaF.NFe.InfAdic.infAdFisco := NotaF.NFe.InfAdic.infAdFisco +
          'PERMITE O APROVEITAMENTO DO CRÉDITO DE ICMS NO VALOR DE R$ ' +
          FormatFloat('R$ #,##0.00', vlrTotCreditoICMS) +
          ' , CORRESPONDENTE À ALÍQUOTA DE 1,25%, NOS TERMORS DO Art.  23, da LC nº 123/06.. MERCADORIA COM ICMS RETIDO POR SUBSTITUICAO TRIBUTARIA.';
      end;
      ...
      // ObsFisco := NotaF.NFe.InfAdic.obsFisco.New;
      // ObsFisco.xCampo := 'ObsFisco';
      // ObsFisco.xTexto:= ObsFisco.xTexto + 'Caso o destinatário seja contribuinte do ICMS,';
      // ObsFisco := NotaF.NFe.InfAdic.obsFisco.New;
      // ObsFisco.xCampo := 'ObsFisco';
      // ObsFisco.xTexto:= ObsFisco.xTexto+' é obrigatória a emissão de NF-e de entrada';
      // ObsFisco := NotaF.NFe.InfAdic.obsFisco.New;
      // ObsFisco.xCampo := 'ObsFisco';
      // ObsFisco.xTexto:=('art. 35, § 5º, do Anexo I do Livro VI do RICMS).');
    end;
    // verifica se é devolução e faz com q o campo adiconal da nota receba os valores
    if categoria_CFOP = 'DV' then
    begin
...
begin
  ...
  Except
    on E: Exception do
    begin
      lblRetorno.Caption := 'Erro ao Encerrar o Manifesto Eletrônico.';
      mmoErro.Lines.Add(E.Message);
      btnOK.Enabled := True;
    end;
  end;
...
begin
  try
    ...
    begin
      ...
      if DM.fdsConfigMDFeTIPO_EMITENTE.AsString = 'teTranspCargaPropria' then
        ...
        // caique disse q não é necessário:se teTransportadora deve antes de emitir o manifesto emitir o CTe das NFes que serão transportadas e depois emitir o MDFe puxando esses CTes
      else
      ...
      Ide.cMDF := cdsManifestoNUM_ALEATORIO_MDFE.AsInteger; // Código Aleatório
      ...
      Begin
        ...
        // obrigatório transpordadores de carga de terceiros
        rodo.CIOT := qryManifesto.FieldByName('CIOT').AsString;
        // obrigatório para veículos de terceiros
      ...
      begin
        ...
        Begin
          ...
          rodo.veicTracao.prop.xNome := qryManifesto.FieldByName('RAZAO_SOCIAL')
            .AsString;
          // nÃO É OBRIGATÓRIO PARA EMITENTE TRANSPORTADOR DA PRÓPRIA CARGA
      ...
      Begin
        ...
        begin
          xNome := qryManifesto.FieldByName('NOME').AsString; // obrigatório
          CPF := qryManifesto.FieldByName('CPF').AsString; // obrigatório
        end;
      End;
      /// ///////////////////////////////////////
      /// Dados do Reboque
      ...
      begin
        ...
        Begin
          ...
          begin
            ...
            // Se for do emitente ou o check na aba de reboque de 'informar proprieetário' estiver desmarcado não é necessário informar
            begin
  ...
  Except
    ...
    begin
      ...
      lblRetorno.Caption := 'Erro ao Enviar Manifesto Eletrônico para o SEFAZ.';
      mmoErro.Lines.Add(E.Message);
      btnOK.Enabled := True;
    end;
  end;
...
begin
  try
    ...
    begin
     ...
     //calculo de IPI
     Produto.Imposto.IPI.CST      := ipi99;//tributação Normal
     if (cdsItensNFeICMS_VIPI.AsFloat>0) and (categoria_CFOP = 'DV') then
     begin
       Produto.infAdProd :=cdsItensNFeINFO_ADICIONAL.AsString+#13'Valor do IPI Devolvido:'+cdsItensNFeICMS_VIPI.AsString+'  Alíquota: '+cdsItensNFeICMS_ALIQIPI.AsString;
    ...
    // Se for devolução, zera (categoria_CFOP = 'DV')
    // if categoria_CFOP = 'DV' then
    // begin
    // NotaF.NFe.Total.ICMSTot.vBCST   := 0;
    // NotaF.NFe.Total.ICMSTot.vST     := 0;
    // end
    // //Se não for pega o valor que foi digitado. Ver essa mesma lógica para os itens
    // else
    // begin
    NotaF.NFe.Total.ICMSTot.vBCST := cdsNFeICMS_ST_VBC.AsFloat;
    NotaF.NFe.Total.ICMSTot.vST   := cdsNFeICMS_ST_VLR.AsFloat;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          /// ////////////////////////////////////////////////////////
           if (qryPedidoVenda.FieldByName('VLR_CARTAO').AsFloat >0) and (cdsCntReceber.Locate('MOEDA',3,[]))then  //pagamento em cartão de débito
           begin
           with NotaF.NFe.pag.New do
            begin
             tPag := TpcnFormaPagamento(3);
      ...
      else // caso não venha de nenhum pedido ou venda
      Begin
        case cdsNFeIDFORMA_PAG.AsInteger of
          1: // Dinheiro
            begin
    ...
    begin
      ...
      // endereço de entrega quando for diferente do Remetente/Destinatário
...
//      ObsComplementar.xTexto := cdsNFeINFO_ADICIONAL.AsString;
    ...
    begin
      ...
      // begin
      //
      // // NotaF.NFe.InfAdic.infAdFisco := NotaF.NFe.InfAdic.infAdFisco +  'DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO SIMPLES NACIONAL.; NÃO GERA DIREITO A CRÉDITO FISCAL DE ICMS, DE ISS E DE IPI; ';
      //
      // end;
      ...
      // ObsComplementar := NotaF.NFe.InfAdic.obsCont.Add;
      // ObsComplementar.xCampo := 'Obs.Fisco';
      // ObsComplementar.xTexto := 'NÃO GERA DIREITO A CRÉDITO FISCAL DE ICMS, DE ISS E DE IPI;';
      //
      // ObsComplementar := NotaF.NFe.InfAdic.obsCont.Add;
      // ObsComplementar.xCampo := 'Teste 2';
      // ObsComplementar.xTexto := 'Descrição do teste 2';
    end;
    // verifica se é devolução e faz com q o campo adiconal da nota receba os valores
    if categoria_CFOP = 'DV' then
    begin
      ...
      begin
        ...
        begin
          Case MessageBox(Application.Handle,
            pchar('Essa nota será emitida em um prazo superior ao permitido pela legislação podendo ou não haver geração de custos.Deseja realmente transmitir essa NFe?'),
...
begin
  ...
  Except
    ...
    begin
      ...
      lblRetorno.Caption := 'Erro ao inutilizar numeração.';
      mmoErro.Lines.Add(E.Message);
      btnOK.Enabled := True;
    end;
  end;
...
function TfrmStatusNFe.TestaConexão: Boolean;
begin
  try
    try
    ...
    Except
      ...
      begin
        lblRetorno.Caption :=
          'Erro de Conexão com o sistema da SEFAZ. Nota emitida Off-Line.';
        mmoErro.Lines.Add
          ('Atençao nota emitida em modo OFF-Line, para evitar multas junto ao SEFAZ, você deve transmitir esta nota em menos de 24h.');
        Result := False;
      end;
    end;
    ...
    Except
      ...
      begin
        ...
        if frmNFe.fdsNFeSTATUS.AsString = 'AB' then
          ...
          lblRetorno.Caption := 'Erro de Conexão com o sistema da SEFAZ.';
      end;
    end;
  finally
    btnOK.Enabled := True;
