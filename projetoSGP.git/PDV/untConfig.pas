...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, RzCmboBx, Data.DB, FIBDataSet,DateUtils,
...
type
  TfrmConfig = class(TForm)
    ...
    cbxVisuCert: TCheckBox;
    RzDBRadioGroup11: TRzDBRadioGroup;
    gbxTipoCaixa: TRzDBRadioGroup;
    mmoLembrete: TRzMemo;
    ...
    procedure cbxVisuCertClick(Sender: TObject);
  private
...
begin
 try
  DM.fdsCaixaCERT_NUM_SERIE.AsString := frmPrincipal.ACBrNFe1.SSL.SelecionarCertificado;
   ...
   begin
     frmPrincipal.imgVencido.Visible :=false;
     frmPrincipal.ImgAVencer.Visible :=false;
   end;
 Except
  on E: Exception do
  begin
  mensagem:= 'Nenhum certificado selecionado:'+ e.Message;
  Application.MessageBox(pChar(mensagem), 'Atenção', MB_OK+MB_ICONQUESTION);
  end;
  End;
end;
...
begin
  ...
  case MessageBox(Application.Handle, Pchar('Deseja realmente salvar as alterações?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    IDYES : begin
  //              if ultimaNFe = 0 then
  //              begin
  //               with qrySQL do
  ...
  //               end;
  //              end;
              frmPrincipal.ACBrNFe1.Configuracoes.Certificados.Senha       := DM.fdsCaixaCERT_SENHA.AsString;
              frmPrincipal.ACBrNFe1.Configuracoes.Certificados.NumeroSerie := DM.fdsCaixaCERT_NUM_SERIE.AsString;
              //frmPrincipal.ACBrNFe1.
...
procedure TfrmConfig.cbxVisuCertClick(Sender: TObject);
begin
if cbxVisuCert.Checked then
 begin
...
begin
  ...
  begin
    case MessageBox(Application.Handle, Pchar('Deseja realmente sair sem salvar as alterações?'), 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
      IDYES : begin
                DM.fdsConfig.Cancel;
...
//  end;
  if Dm.fdsUsuarioPermicIDUSUARIO.AsInteger = 1 then
  begin
    gbxTipoCaixa.Visible:=true;
    mmoLembrete.Visible := True;
  end;
