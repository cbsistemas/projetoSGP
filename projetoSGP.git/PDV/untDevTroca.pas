...
type
  ...
  private
  function verificaDevolver(): boolean;
...
begin
  ...
  Application.MessageBox('Você deve determinar a quantidade a ser devolvida antes de prosseguir.','Atenção',MB_OK+MB_ICONASTERISK)
end;
...
begin
  ...
  //Estava ak a verificação do itemBloqueado
end;
...
//  begin
  if ((cdsItensBloqueados.Locate('ITEM', fdsVendaItensITEM.AsString, [])) and (fdsVendaItensQTN_DEVOLVER.AsFloat > 0)) then
  begin
...
function TfrmDevTroca.verificaDevolver: boolean;
begin
 result:=false;
 fdsvendaItens.First;
 while not fdsVendaItens.Eof do
