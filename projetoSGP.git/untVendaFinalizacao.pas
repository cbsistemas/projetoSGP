...
begin
  ...
  begin
    ...
    begin
      TfrmMenssagens.Mensagem('O limite de crediário para esse cliente é de R$'+FloatToStr(fdsClienteLIMITE_CREDITO.AsFloat), 'Cliente Bloqueado','T',[MBOK]);

      temPermicao := uVerificaPermicao.Verifica('LIBERAR_CLIENTE_BLOQ');

      if not(temPermicao) then
      begin
        TFrmMenssagens.Mensagem('Desculpe,usuário sem permissão para ultrapassar o limite de crediário de cliente.', 'Crediário não permitido.','I',[mbOk]);
        abort;
      end;
    end;
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar a finalização da Venda?'), 'Atenção',
...
procedure TfrmVendaFinalizacao.edtNotaEnter(Sender: TObject);
  var
  utils : TUtils;
begin                               //não existia vvc quem colocu
  edtCartao.Enabled := false;
  edtParcelasCT.Enabled := false;
  edtCheque.Enabled := false;
  edtParcelasCH.Enabled := false;
