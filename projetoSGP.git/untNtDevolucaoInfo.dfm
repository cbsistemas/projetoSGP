object frmNtDevolucaoInfo: TfrmNtDevolucaoInfo
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Nota Fiscal de Devolu'#231#227'o - Dados adicionais'
  ClientHeight = 454
  ClientWidth = 534
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  DesignSize = (
    534
    454)
  PixelsPerInch = 96
  TextHeight = 13
  object RzBackground1: TRzBackground
    Left = 0
    Top = 0
    Width = 534
    Height = 454
    Active = True
    Align = alClient
    Anchors = [akLeft, akTop, akRight]
    GradientColorStart = -1
    GradientColorStop = 2497553
    GradientDirection = gdDiagonalDown
    ImageStyle = isCenter
    ShowGradient = True
    ShowImage = False
    ShowTexture = False
    ExplicitLeft = 109
    ExplicitWidth = 433
  end
  object RzLabel22: TRzLabel
    Left = 8
    Top = 76
    Width = 99
    Height = 13
    Anchors = [akLeft]
    Caption = 'Chave a Devolver'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Transparent = True
  end
  object RzLabel2: TRzLabel
    Left = 295
    Top = 76
    Width = 39
    Height = 13
    Anchors = [akLeft]
    Caption = 'N'#186' NFe '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Transparent = True
  end
  object RzPanel1: TRzPanel
    Left = 0
    Top = 0
    Width = 534
    Height = 65
    Anchors = [akLeft, akTop, akRight]
    BorderOuter = fsNone
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    DesignSize = (
      534
      65)
    object lblTituloTotal: TRzLabel
      Left = 734
      Top = 16
      Width = 41
      Height = 17
      Alignment = taRightJustify
      Caption = 'Total:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial Rounded MT Bold'
      Font.Style = [fsUnderline]
      ParentFont = False
    end
    object lblTotal: TRzLabel
      Left = 765
      Top = 39
      Width = 10
      Height = 17
      Alignment = taRightJustify
      Caption = '--'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -15
      Font.Name = 'Arial Rounded MT Bold'
      Font.Style = [fsUnderline]
      ParentFont = False
    end
    object RzLabel1: TRzLabel
      Left = 8
      Top = 16
      Width = 56
      Height = 13
      Anchors = [akLeft]
      Caption = 'Dt. Emiss'#227'o'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object dtpDtEmissao: TJvDateTimePicker
      Left = 8
      Top = 35
      Width = 91
      Height = 21
      CalColors.BackColor = clCream
      CalColors.TitleBackColor = clMoneyGreen
      CalColors.TrailingTextColor = clMoneyGreen
      Date = 42964.661904375000000000
      Time = 42964.661904375000000000
      Color = clMoneyGreen
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = False
      TabOrder = 0
      DropDownDate = 42964.000000000000000000
      HintColor = clMoneyGreen
    end
  end
  object mmoInfo: TRzMemo
    Left = 8
    Top = 274
    Width = 520
    Height = 130
    Anchors = [akLeft, akTop, akRight]
    Color = clMoneyGreen
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
  end
  object btnOK: TRzBitBtn
    Left = 434
    Top = 421
    Width = 92
    Anchors = [akRight, akBottom]
    Caption = 'Con&firmar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    OnClick = btnOKClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object RzBitBtn1: TRzBitBtn
    Left = 347
    Top = 421
    Width = 81
    Anchors = [akRight, akBottom]
    Caption = '&Cancelar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    OnClick = RzBitBtn1Click
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object JvDBGrid1: TJvDBGrid
    Left = 8
    Top = 122
    Width = 520
    Height = 146
    Anchors = [akLeft, akTop, akRight]
    DataSource = frmNfe.dsNF_Devolvidas
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    ReadOnly = True
    TabOrder = 4
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    SelectColumnsDialogStrings.Caption = 'Select columns'
    SelectColumnsDialogStrings.OK = '&OK'
    SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
    EditControls = <>
    RowsHeight = 17
    TitleRowHeight = 17
    Columns = <
      item
        Expanded = False
        FieldName = 'CHAVE_NF_COMPRA'
        Title.Caption = 'Chave NF a Devolver'
        Width = 390
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOTA_NUM'
        Title.Caption = 'N'#186' NFe'
        Width = 88
        Visible = True
      end>
  end
  object btnIncChave: TRzBitBtn
    Left = 392
    Top = 91
    Width = 29
    Hint = 'Cadastrar novo Ve'#237'culo'
    FrameColor = 7617536
    Anchors = [akLeft, akBottom]
    Color = 15791348
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    HotTrack = True
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 5
    OnClick = btnIncChaveClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object btnEditChave: TRzBitBtn
    Left = 427
    Top = 91
    Width = 29
    Hint = 'Alterar Ve'#237'culo selecionado'
    FrameColor = 7617536
    Anchors = [akLeft, akBottom]
    Color = 15791348
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    HotTrack = True
    ParentFont = False
    TabOrder = 6
    OnClick = btnEditChaveClick
    Glyph.Data = {
      
      E8101717101010101010E8E8E8E8E8E8E8568181DFDFDFDFDFDF}
    NumGlyphs = 2
  end
  object btnDeleteChave: TRzBitBtn
    Left = 462
    Top = 91
    Width = 29
    Hint = 'Apagar Ve'#237'culo selecionado'
    FrameColor = 7617536
    Anchors = [akLeft, akBottom]
    Color = 15791348
    HotTrack = True
    TabOrder = 7
    OnClick = btnDeleteChaveClick
    Glyph.Data = {
      
      57A82B572C5757E8E8E8E8E8E8E8E88181E28181818181E8E8E8}
    NumGlyphs = 2
  end
  object btnGravaChave: TRzBitBtn
    Left = 497
    Top = 91
    Width = 29
    Hint = 'Apagar Ve'#237'culo selecionado'
    FrameColor = 7617536
    Anchors = [akLeft, akBottom]
    Color = 15791348
    Enabled = False
    HotTrack = True
    TabOrder = 10
    OnClick = btnGravaChaveClick
    Glyph.Data = {
      
      E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object edtChaveDevolver: TRzDBEdit
    Left = 8
    Top = 95
    Width = 281
    Height = 21
    DataSource = frmNfe.dsNF_Devolvidas
    DataField = 'CHAVE_NF_COMPRA'
    ReadOnly = True
    Color = clInfoBk
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    FocusColor = clMoneyGreen
    ParentFont = False
    TabOrder = 8
  end
  object edtNotaNUm: TRzDBEdit
    Left = 295
    Top = 95
    Width = 93
    Height = 21
    DataSource = frmNfe.dsNF_Devolvidas
    DataField = 'NOTA_NUM'
    ReadOnly = True
    Color = clInfoBk
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    FocusColor = clMoneyGreen
    ParentFont = False
    TabOrder = 9
  end
  object fdsCFOP: TpFIBDataSet
    UpdateSQL.Strings = (
      'UPDATE CFOP'
      'SET '
      '    CODIGO_CFOP = :CODIGO_CFOP'
      'WHERE'
      '    ID = :OLD_ID'
      '    ')
    DeleteSQL.Strings = (
      'DELETE FROM'
      '    CFOP'
      'WHERE'
      '        ID = :OLD_ID'
      '    ')
    InsertSQL.Strings = (
      'INSERT INTO CFOP('
      '    ID,'
      '    CODIGO_CFOP'
      ')'
      'VALUES('
      '    :ID,'
      '    :CODIGO_CFOP'
      ')')
    RefreshSQL.Strings = (
      'SELECT'
      '    ID,'
      '    CODIGO_CFOP,'
      '    CODIGO_CFOP || '#39' - '#39' || NOME AS COD_NOME'
      'FROM'
      '    CFOP '
      'WHERE( '
      
        '   CODIGO_CFOP IN ('#39'5201'#39', '#39'6201'#39', '#39'5202'#39', '#39'6202'#39', '#39'5553'#39', '#39'6553' +
        #39', '#39'5556'#39', '#39'6556'#39')'
      '     ) and (     CFOP.ID = :OLD_ID'
      '     )'
      '     ')
    SelectSQL.Strings = (
      'SELECT'
      '    ID,'
      '    CODIGO_CFOP,'
      '    CODIGO_CFOP || '#39' - '#39' || NOME AS COD_NOME'
      'FROM'
      '    CFOP '
      'WHERE'
      '   CODIGO_CFOP IN  ')
    Transaction = DM.transPricipal
    Database = DM.Conexao
    Left = 208
    Top = 8
    object fdsCFOPID: TFIBBCDField
      FieldName = 'ID'
      Size = 0
    end
    object fdsCFOPCOD_NOME: TFIBStringField
      FieldName = 'COD_NOME'
      Size = 257
      Transliterate = False
      EmptyStrToNull = True
    end
    object fdsCFOPCODIGO_CFOP: TFIBStringField
      FieldName = 'CODIGO_CFOP'
      Size = 4
      Transliterate = False
      EmptyStrToNull = True
    end
  end
  object dsCFOP: TDataSource
    DataSet = fdsCFOP
    Left = 216
    Top = 56
  end
  object qryComand: TpFIBQuery
    Transaction = DM.transPricipal
    Database = DM.Conexao
    Left = 149
    Top = 8
  end
end
