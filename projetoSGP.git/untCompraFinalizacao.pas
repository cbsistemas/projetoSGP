...
type
  ...
  public
  var
    idCompra, idPagar, idFormaPag, idFornecedor, idBanco, nBanco, nAgencia,
      nConta, parcelas: Integer;
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar a finalização da Compra?'), 'Atenção',
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Application.MessageBox('Você deve preencher o nº de todos os cheques.',
...
begin
  ...
  begin
    ...
    cdsPagarChequePARCELA.AsString := utils.FormataParcelas(IntToStr(i)) +
      parcelasTemp;
...
begin
  ...
  begin
    ...
    cdsPagarItemPARCELA.AsString := utils.FormataParcelas(IntToStr(i)) +
      parcelasTemp;
...
begin
  ...
  begin
    ...
    while not(cdsPagarCheque.Eof) do
    begin
      GravaCntPagarItem('CH', cdsPagarChequePARCELA.AsString,
        cdsPagarChequeDT_VENCIMENTO.AsDateTime, cdsPagarChequeVALOR.AsFloat);
      idPagarItem := RetornaCntPagarItemID(cdsPagarChequeIDFILIAL.AsInteger,
        cdsPagarChequeVALOR.AsFloat, cdsPagarChequeDT_VENCIMENTO.AsDateTime);
      ...
      qryInsertChequeCntPagar.ParamByName('IDFILIAL').AsInteger :=
        cdsPagarChequeIDFILIAL.AsInteger;
      qryInsertChequeCntPagar.ParamByName('IDBANCO').AsString :=
        IntToStr(idBanco);
      qryInsertChequeCntPagar.ParamByName('AGENCIA').AsString :=
        IntToStr(nAgencia);
  ...
  Begin
    ...
    begin
      ...
      begin
        GravaCntPagarItem(prefixoTemp, cdsPagarItemPARCELA.AsString,
          cdsPagarItemDT_VENC.AsDateTime, cdsPagarItemVALOR.AsFloat);
        cdsPagarItem.Next;
        prefixoTemp := cdsPagarItemPREFIXO.AsString;
      end;
      moeda := prefixoTemp;
