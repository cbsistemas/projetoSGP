...
begin
  ...
  begin
   ...
   TfrmMenssagens.Mensagem('Desculpe, usuário sem permissão para alterar NFe.','Sem Permissão','I',[mbOk]);
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar a Inclusão/Alteração desta NFe?'),
...
begin
  ...
  begin
    ...
    begin
      ...
      Application.MessageBox('Nota excluída com sucesso!', 'Sucesso',
      ...
      Application.MessageBox('Você não pode excluir essa nota!Tente inutilizar',
        'Atenção', MB_ICONERROR + MB_OK);
  end
  else
  TfrmMenssagens.Mensagem('Desculpe, usuário sem permissão para Excluir NFe.','Sem Permissão','I',[mbOk]);
end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Você está incluindo ou alterando um item, grave a inclusão/alteração deste primeiro e depois grave a Nota.',
  ...
  begin
    Application.MessageBox('Você não pode gravar uma Nota sem itens vendidos.',
      'Anteção', MB_OK + MB_ICONERROR);
...
begin
  ...
  begin
    Application.MessageBox
      ('Antes Selecione uma Natureza de Operação',
      'Anteção', MB_OK + MB_ICONEXCLAMATION);
  ...
  begin
    ...
    begin
      try
        ...
        frmStatusNFe.lblRetorno.Caption := 'Aguarde enviando Carta de Correção';
  ...
  begin
    case fdsNFe.State of
      dsInsert:
        begin
          mensagem :=
            'Salve ou Cancele a Inclusão da Nota Fiscal antes de sair.'
        end;
      dsEdit:
        begin
          mensagem :=
            'Salve ou Cancele a Alteração da Nota Fiscal antes de sair.'
        end;
    end;
  ...
  begin
    ...
    begin
      ...
      cdsNFe_ItemVLR_UNITARIO.AsFloat := qrySql.FieldByName('PREC_VENDA')
        .AsFloat; // vitor ver sobre tabela de preço
      cdsNFe_ItemCOD_FABRICA.AsString :=
...
begin
 ...
 begin
    ...
    begin
      //colocar para tirar a diferença do ultimo;
...
begin
  ...
  Finally
    ...
    begin
      ...
      transNFe.CommitRetaining;
      Application.MessageBox
        ('Operação finalizada com sucesso!Clique em OK para continuar.',
    ...
    begin
      ...
      Finally
        ...
        begin
          ...
          // fdsPedidoOBS.AsString := 'Nº NF Devolvida: ' + frmNtDevolucaoInfo.edtNumNfe.Text + ' - Dt. Emissão: ' + FormatDateTime('MM/DD/YYYY',frmNtDevolucaoInfo.dtpDtEmissao.Date) + ' - Motivo: ' + frmNtDevolucaoInfo.mmoInfo.Text;

          if fdsNatOperacaoCAT_SAIDA_CFOP.AsString = 'DV' then
            fdsNFeOBS.AsString := 'Motivo Devolução: ' +
