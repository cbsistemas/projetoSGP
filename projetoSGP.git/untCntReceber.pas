...
begin
  ...
  begin
    ...
    ('Por favor, informe um cliente para e realização do Recebimento.',
  ...
  begin
    Application.MessageBox('Por favor,selecione as notas para a realização do Recebimento.',
      'Atenção',MB_OK+MB_ICONASTERISK);
    abort;
  end;
  Try
  ...
  Finally
    ...
    begin
      ...
      DM.valor :=FloatToStr((frmPagIncReceber.valorDinheiro +frmPagIncReceber.valorCheque
        + frmPagIncReceber.valorCartao) - (frmPagIncReceber.troco)); //Retirei o crédito pq eraldo reclamou
      frmPagIncReceber.cdsDocumentos.First;
      with fdsMovCntReceber do
       begin
         ...
         Append;
          ...
          fdsMovCntReceberVLR_TOT_RECEBIDO.AsFloat :=(frmPagIncReceber.valorDinheiro +frmPagIncReceber.valorCartao + frmPagIncReceber.valorCheque+ frmPagIncReceber.valorParcelar); //Retirei o crédito pq eraldo reclamou
      ...
      case MessageBox(Application.Handle,pchar('Deseja imprimir o comprovante?'),'Atenção',MB_YESNO+MB_ICONQUESTION) of
        IDYES:begin
...
begin
  ...
  begin
    Application.MessageBox
    ('Por favor, informe um cliente para a realização da renegociaçõa.',
    'Atenção',MB_OK+MB_ICONASTERISK);
    abort;
  end;
  if vlrSelecionado <= 0 then
  begin
    Application.MessageBox('Por favor, selecione as notas para a realização da renegociação.',
      'Atenção', MB_OK+MB_ICONASTERISK);
    abort;
  end;
  Try
    fdsCntReceberItens.First;
    historico:='Renegociação referente as parcelas: ';
    frmPagIncReceber := TfrmPagIncReceber.Create(nil);
    frmPagIncReceber.lblTitulo.Caption := 'Renegociação de Notas de Cliente';
    while not(fdsCntReceberItens.Eof) do
    begin
      if fdsCntReceberItensBAIXA_AGRUPADA.AsString = 'S' then
      begin
    ...
    begin
     ...
     try
       ...
       DM.transPricipal.CommitRetaining;
        ...
        Application.MessageBox('Renegociação Realizada com Sucesso!', 'Conta Renegociada.',
        MB_OK + MB_ICONEXCLAMATION);
        case MessageBox(Application.Handle,pchar('Deseja imprimir o comprovante de Renegociação?'),'Atenção',MB_YESNO+MB_ICONQUESTION) of
        IDYES:begin
...
begin
  ...
  begin
    ...
    if (rgbClienteMov.Checked) and not(Trim(edtPesqClienteMov.Text) = 'TODOS') and not(Trim(edtCodigoClienteMov.Text) = 'CÓDIGO') then
    begin
...
begin
  if Trim(edtClienteCodigo.Text) = 'CÓDIGO' then
  begin
...
begin
  if Trim(edtCodigoClienteMov.Text) = 'CÓDIGO' then
  begin
...
begin
  ...
  begin
    edtCodigoClienteMov.Text := 'CÓDIGO';
...
begin
  if Trim(edtCodigoClienteRene.Text) = 'CÓDIGO' then
  begin
...
begin
  ...
  begin
    edtCodigoClienteRene.Text := 'CÓDIGO';
...
begin
  ...
  Case MessageBox(Application.Handle,
      Pchar('Deseja selecionar um cliente neste momento?'), 'Atenção',
...
begin
  ...
  // showmessage('Coluna: ' + intToStr(ColunaClicada) + '   Título: ' + gdrReceberAnalitico.Columns[ColunaClicada].Title.Caption + '    Field: ' + gdrReceberAnalitico.Columns[ColunaClicada].FieldName);
  ...
  begin
   ...
   begin
     Application.MessageBox('É necessário informar um cliente para efetuar o recebimento.', 'Atenção.',
      MB_OK + MB_ICONEXCLAMATION);
      abort;
   end;
    if fdsCntReceberItensPOSICAO.AsString = 'AB' then
 ...
 begin
  ...
  begin
    ...
    cdsFormaPagamentoVALOR.AsFloat :=
        ...
        +fdsMovCntReceberVLR_DINHEIRO.AsFloat); //Retirei o crédito pq eraldo reclamou
    Post;
...
begin
  ...
  begin
    ...
    Append;
    cdsDescAcreMOEDA.AsString := 'Valor Líquido:';
  ...
  begin
    ...
    begin
      Append;
      cdsFormaPagamentoMOEDA.AsString := 'Cartão Crédito: ';
    ...
    begin
      Append;
      cdsFormaPagamentoMOEDA.AsString := 'Cartão Débito: ';
...
begin
  ...
  begin
    ...
    // Aqui pego o Nosso Número
    linha := 5;
    coluna := 1;
    ShowMessage(IntToStr(i) + ' - Doc Número: ' + sheet.cells[linha,
      coluna].Text);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DM.obs:=' Lamentamos que V.Sa não tenha comparecido em nosso departamento de crédito e cobrança,';
        DM.Obs:= DM.Obs +' para buscar uma solução para o seu débito.'+#13+'  V.Sa disporá ainda de 72 horas(três dias úteis)';
        DM.Obs:= DM.Obs +' para solucionar esta pendência diretamente em nossa loja.'+ #13;
        DM.Obs:= DM.Obs +'Fim tal praso nos consideramos liberados para a tomada das medidas necessárias, ou seja, registro no Serviço';
        DM.Obs:= DM.Obs +' de Proteção ao Crédito(SPC) do Brasil Ltda, e cobrança judicial.'+#13;
        DM.Obs:= DM.Obs +'Caso já tenha efetuado o pagamento favor desconsiderar esta.';
        DM.frxReport1.Clear;
      ...
      begin
        Application.MessageBox('Não foram selecionadas notas para a impressão de cobrança.','Atenção',MB_OK+MB_ICONEXCLAMATION);
      end;
    end
    else
    begin
      Application.MessageBox('Antes informe um cliente.','Atenção',MB_OK+MB_ICONEXCLAMATION);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if fdsCntReceberItens.RecordCount>0 then
      begin
        DM.carta:='MODELO AMIGÁVEL';
        DM.obs:='Caso já tenha efetuado o pagamento favor desconsiderar esta.'+#13+#13+'  VENHA NEGOCIAR SUA DÍVIDA!';
        DM.frxReport1.Clear;
        DM.frxReport1.LoadFromFile(ExtractFilePath(Application.ExeName) +'/Relatorios/frxRelCartaCobranca.fr3');
        DM.frxReport1.PrintOptions.ShowDialog := True;
        DM.frxReport1.PrepareReport(True);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        DM.carta:='MODELO SPC';
        DM.obs:='Como não compreceu para negociar seu débito, seu nome foi inserido no SPC/SERASA';
        DM.Obs:= DM.Obs +' assim sendo fica impossibilitado de utilizar seu crédito em outras entidades.';
        DM.Obs:= DM.Obs +' Estamos lhe esperando para renegociarmos seu débito perante a loja, caso ja tenha efetuado o pagamento favor desconsiderar esta.';
        DM.Obs:= DM.Obs +#13+#13+' OBRIGADA A GERÊNCIA.';
        DM.frxReport1.Clear;
        DM.frxReport1.LoadFromFile(ExtractFilePath(Application.ExeName) +'/Relatorios/frxRelCartaCobranca.fr3');
        DM.frxReport1.PrintOptions.ShowDialog := True;
        DM.frxReport1.PrepareReport(True);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DM.obs:='Estamos verificando através de sua ficha de Crédito que v.sa está em atraso.';
        DM.Obs:= DM.Obs +' Acreditamos que algo tenha acontecido, por isso gstaríamos de merecer sua justificativa pessoal.'+#13;
        DM.Obs:= DM.Obs +'Caso não compareça no prazo de até 5 dias úteis a partir do recebimento desta carta, não nos responsabilizaremos';
        DM.Obs:= DM.Obs +' pelo envio dos seus dados para entidades tipo SPC e SERASA.'+#13;
        DM.Obs:= DM.Obs +'Sem mais para o momento, departamento de crédito e cobrança.'+#13#13;
...
begin
  ...
  begin
    Case MessageBox(Application.Handle,
      Pchar('Deseja realmente excluir essa Conta a Receber?'), 'Atenção',
      MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
      idYes:
        begin
          with qryComando do
    ...
    Application.MessageBox
      ('Você não pode Excluir uma conta que já tenha tido recebimento.',
...
procedure TfrmCntReceber.btnCancelarReceberMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
 btnCancelarReceber.Hint:='Só será possível excluir notas que ainda não foram '+#13+'efetuadas nenhuma forma pagamento.'
end;
...
begin
  Try
    ...
    frmPagIncReceber.modo := 'Inclusão';
...
begin
  ...
  begin
   ...
   begin
     close;
     Conditions.Clear;
     if (edtCodigoClienteMov.Text<>'CÓDIGO') AND (edtCodigoClienteMov.Text<>EmptyStr) then
     begin
  ...
  begin
   ...
   begin
     close;
     Conditions.Clear;
     if (edtClienteCodigo.Text<>'CÓDIGO') AND (edtClienteCodigo.Text<>EmptyStr) then
     begin
