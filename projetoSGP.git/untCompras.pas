...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir está nota de compra?'), 'Atenção',
    MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2) of
    idYes:
      begin
        DeletarCompra;
        Application.MessageBox('Compra excluída com sucesso!', 'Sucesso',
...
begin
  ...
  begin
    Application.MessageBox
      ('Você está incluindo ou alterando um item, grave a inclusão/alteração deste primeiro e depois grave a compra.',
  ...
  begin
    ...
    begin
      Application.MessageBox('Você não pode gravar uma Compra sem itens.',
        'Anteção', MB_OK + MB_ICONERROR);
    ...
    Case MessageBox(Application.Handle,
      Pchar('Deseja lançar os dados de pagamento agora?'), 'Atenção',
      ...
      IDNO:
        begin
          ...
          DM.Conexao.CommitRetaining;
          Application.MessageBox('Compra lançada com sucesso.', 'Atenção',
            MB_ICONEXCLAMATION + MB_OK);
    ...
    Case MessageBox(Application.Handle,
      Pchar('Deseja Imprimir Etiqueta dos Produtos?'), 'Atenção',
      MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON1) of
      idYes:
      begin
        Try
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox
        ('Esta nota fiscal nao é do seu CNPJ, escolha outra nota.', 'Atenção',
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox
            ('Esta nota já foi lançada no sistema, escolha outra nota.',
      ...
      begin
        ...
        begin
          ...
          cdsCompraItemOBS.AsString := 'Custo =(Custo Unitário='+FloatToStr(Produto.Prod.vUnCom)+')+(FCP='+ FloatToStr(cdsCompraItemVLR_FUNDO_POBREZA.AsFloat)+')+(IPI='+FloatToStr(cdsCompraItemVLR_IPI_PROD.AsFloat)+
          ...
          begin
            ...
            // cdsCompraItemSITUACAO_SISTEMA.AsString := 'NãO ENCONTRADO';
          ...
          begin
            ...
            cdsCompraItemINFORMACAO.AsString:='Preço:R$'+qrySql.FieldByName('PREC_VENDA').AsString+' Preço de Custo:R$'+qrySql.FieldByName('PREC_CUSTO').AsString+' Custo Médio:R$'+qrySql.FieldByName('PREC_CUSTO_MEDIO').AsString;
            with cdsCadadastroItens do
            begin
              Open;
              Append;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Case MessageBox(Application.Handle,
          Pchar('Preço de Venda informado é menor que o Preço de Custo, deseja gravar assim mesmo?'),
...
begin
  ...
  transProduto.CommitRetaining;
   ...
   //alocando os produtos que vão ser castrados, para caso a nota de entrada seja cancelada os produtos sejam excluidos
    ...
    begin
      ...
      begin
        ...
        SQL.Add('execute procedure gera_mov_item(' +
          ...
          DM.fdsConfigIDFILIAL.AsString + ',CURRENT_DATE,' +
          QuotedStr('NOTA COMPRA EXCLUÍDA') + ',' + fdsComprasID.AsString + ','
...
begin
  ...
  cdsCompraItemINFORMACAO.AsString:='Preço:R$'+fdsProdutoItemPREC_VENDA.AsString+' Preço de Custo:R$'+fdsProdutoItemPREC_CUSTO.AsString+' Custo Médio:R$'+fdsProdutoItemPREC_CUSTO_MEDIO.AsString;
...
begin
  Try
    ...
    frmCompraFinalizacao.edtDescricao.Text := 'Compra Nº ' +
  ...
  Finally
    ...
    begin
      Application.MessageBox('Finalização de compra cancelada.', 'Atenção',
        MB_ICONEXCLAMATION + MB_OK);
    end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Salve ou Cancele o lançamento da nota antes de sair.', 'Atenção',
      MB_OK + MB_ICONWARNING);
    Abort;
  end
  else
  ...
  begin
    ...
    Finally
      ...
      begin
        ...
        begin
          Case MessageBox(Application.Handle,
            Pchar('Este item já foi incluso no Pedido, deseja duplicá-lo?'),
            'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
            IDNO:
              begin
                cdsCompraItem.Cancel;
...
procedure TfrmCompras.RzSplitter2MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  RzSplitter2.Hint:='Atenção|Nesta aba consta Informação dos Impostos refente ao Cadastro do Produto.'+#13+'As alterações feitas aqui implicara na mudança no cadastro do produto.'+#13+'|0';
end;
