...
uses
  ...
  Vcl.Mask, RzEdit, RzDBEdit, RzTabs, RzBckgnd, Data.DB, FIBDataSet,
  pFIBDataSet,
  Datasnap.Provider, Datasnap.DBClient, pFIBClientDataSet, FIBQuery, pFIBQuery,
  System.Math,
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar a  Inclusão/Alteração do Romaneio atual?'),
      ...
      begin
        ...
        begin
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('ALTER SEQUENCE GEN_ID_ROMANEIO_CNTRECEBER RESTART WITH ' +
              IntToStr(maxID));
            Prepare;
            ExecQuery;
          end;
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente EXCLUIR o Romaneio atual?'), 'Atenção',
...
begin
  EditaInsere;
  Application.MessageBox('Informe o valor recebido para cada nota do romanio.',
    'Finalização do Romaneio', MB_OK + MB_ICONEXCLAMATION);
  cdsRomCntReceber.Edit;
  ...
  begin
    try
      ...
      begin
        ...
        cdsItemRomCntReceberVLR_RECEBIDO.AsFloat :=
          frmPagIncReceber.valorDinheiro + frmPagIncReceber.valorCheque +
          frmPagIncReceber.valorCartao;
        cdsItemRomCntReceber.Post;

        cdsRomCntReceberVLR_TOTAL_RECEBIDO.AsFloat :=
          cdsRomCntReceberVLR_TOTAL_RECEBIDO.AsFloat +
          cdsItemRomCntReceberVLR_RECEBIDO.AsFloat;
      end;
  ...
  Application.MessageBox('Notas recebidas e Romaneio finalizado com sucesso!',
    'Finalização do Romaneio', MB_OK + MB_ICONEXCLAMATION);
  btnGravar.Click;
end;

procedure TfrmRomaneioCntReceber.btnFirstClick(Sender: TObject);
begin
  cdsRomCntReceber.Prior;
end;
...
begin
  ...
  EditaInsere;
  cdsItemRomCntReceber.EmptyDataSet;
  with qryComand do
  begin
    Close;
    SQL.Clear;
...
begin
  ...
  DM.frxReport1.LoadFromFile(ExtractFilePath(Application.ExeName) +
    '/Relatorios/frxRomaneioCntReceber.fr3');
...
procedure TfrmRomaneioCntReceber.btnLastClick(Sender: TObject);
begin
  cdsRomCntReceber.Last;
end;

procedure TfrmRomaneioCntReceber.btnNextClick(Sender: TObject);
begin
  cdsRomCntReceber.Next;
end;

procedure TfrmRomaneioCntReceber.btnPedidoExcluirClick(Sender: TObject);
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir esta nota do Romaneio atual?'), 'Atenção',
    MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    idYes:
      begin
        cdsRomCntReceberVLR_TOTAL_RECEBER.AsFloat :=
          cdsRomCntReceberVLR_TOTAL_RECEBER.AsFloat -
          cdsItemRomCntReceberVLR_RECEBER.AsFloat;
        cdsRomCntReceberQTN_NOTAS.AsInteger :=
          cdsRomCntReceberQTN_NOTAS.AsInteger - 1;
        cdsItemRomCntReceber.Delete;
...
begin
  ...
  Finally
    ...
    begin
      ...
      begin
        if frmPesqCntReceber.fdsCntReceberItensBAIXA_AGRUPADA.AsString = 'S'
        then
        begin
          if cdsItemRomCntReceber.Locate('REFERENCIA',
            frmPesqCntReceber.fdsCntReceberItensREFERENCIA.AsString, []) then
          begin
            Application.MessageBox
              (Pchar('A Nota ' + frmPesqCntReceber.fdsCntReceberItensREFERENCIA.
              AsString + ' já está inclusa neste Romaneio.'), 'Anteção',
              MB_OK + MB_ICONERROR);
          end
          else
          begin
            vlrReceber :=
              RoundTo(frmPesqCntReceber.fdsCntReceberItensVALOR.AsFloat -
              frmPesqCntReceber.fdsCntReceberItensVALOR_PAGO.AsFloat -
              frmPesqCntReceber.fdsCntReceberItensDESCONTO.AsFloat, -2);
            cdsItemRomCntReceber.Append;
            cdsItemRomCntReceberREFERENCIA.AsString :=
              frmPesqCntReceber.fdsCntReceberItensREFERENCIA.AsString;
            cdsItemRomCntReceberIDROMANEIO.AsInteger :=
              cdsRomCntReceberID.AsInteger;
            cdsItemRomCntReceberIDRECITEM.AsInteger :=
              frmPesqCntReceber.fdsCntReceberItensID.AsInteger;
            cdsItemRomCntReceberIDFILIAL.AsInteger :=
              cdsRomCntReceberIDFILIAL.AsInteger;
            cdsItemRomCntReceberVLR_RECEBER.AsFloat := vlrReceber;
            cdsItemRomCntReceberVLR_RECEBIDO.AsFloat := 0;
            cdsItemRomCntReceberCLIENTE.AsString :=
              frmPesqCntReceber.fdsCntReceberItensCLIENTE.AsString;
            cdsItemRomCntReceber.Post;

            cdsRomCntReceberVLR_TOTAL_RECEBER.AsFloat :=
              cdsRomCntReceberVLR_TOTAL_RECEBER.AsFloat + vlrReceber;
            cdsRomCntReceberQTN_NOTAS.AsInteger :=
              cdsRomCntReceberQTN_NOTAS.AsInteger + 1;

            frmPesqCntReceber.fdsCntReceberItens.Edit;
            frmPesqCntReceber.fdsCntReceberItensBAIXA_AGRUPADA.AsString
              := EmptyStr;
...
procedure TfrmRomaneioCntReceber.btnPriorClick(Sender: TObject);
begin
  cdsRomCntReceber.First;
end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Salve ou Cancele a Inclusão/Alteração do Romaneio antes de sair.',
...
begin
  if not(cdsRomCntReceber.State in [dsInsert, dsEdit]) and
    (cdsRomCntReceber.RecordCount > 0) then
  begin
...
begin
  ...
  begin
    Close;
    Conditions.Clear;
    Conditions.AddCondition('STATUS', 'RCR.STATUS = ' +
