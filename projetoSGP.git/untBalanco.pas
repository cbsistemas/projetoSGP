...
begin
  ...
  begin
    Application.MessageBox
      ('Este Balanço não está mais aberto, você não pode mais alterá-lo.',
      'Não Permitido', MB_OK + MB_ICONEXCLAMATION);
    Abort;
  end;
...
begin
  ...
  begin
    Application.MessageBox
      ('O último Balanço não foi apagado ainda, então, não pode ser recuperado.',
...
begin
  ...
  if (fdsBalancoTIPO.AsString = 'GR') then
    mensagem :=
      'Deseja realmente finalizar este balanço no Estoque Gerencial? O estoque será alterado, não será possível desfazer a operação!'
  else
    mensagem :=
      'Deseja realmente finalizar este balanço no Estoque Real? O estoque será alterado, não será possível desfazer a operação!';
  ...
  begin
    ...
    Case MessageBox(Application.Handle,
      Pchar('Deseja ZERAR o SALDO dos produtos NÃO LANÇADOS no balanço?'),
      'Atenção', MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2) of
      idYes:
        begin
          zerarSaldo := True;
        end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Este Balanço não está mais aberto, você não pode Excluí-lo.',
      'Não Permitido', MB_OK + MB_ICONEXCLAMATION);
    Abort;
  end;

  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir este Balanço?'), 'Atenção',
    MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    idYes:
      begin
        cdsBalancoItens.Close;
        ...
        Application.MessageBox
          ('Balanço apagado com Sucesso, enquanto você não alterar ou iniciar um novo balanço, você poderá recuperar este balanço através do botão recurepação.',
          'Operação realizada', MB_OK + MB_ICONEXCLAMATION);
        GravaCancela;
      end;
  end;
end;
...
begin
  ...
  begin
    Application.MessageBox('Você não pode gravar um Balanço sem itens.',
      'Anteção', MB_OK + MB_ICONERROR);
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir este item do balanço?'), 'Atenção',
    MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    idYes:
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SQL.Add(QuotedStr('BALANÇO - REAL') + ', ' +
      ...
      begin
        SQL.Add('EXECUTE PROCEDURE gera_mov_item(' +
          ...
          cdsBalancoItensIDFILIAL.AsString + ', ' +
          QuotedStr(FormatDateTime('MM/DD/YYYY', Now)) + ', ');
        SQL.Add(QuotedStr('BALANÇO - GERENCIAL') + ', ' +
          cdsBalancoItensIDBALANCO.AsString + ', ' + QuotedStr('B') + ', ' +
          ':QTN, ' + cdsBalancoItensIDFILIAL.AsString + ')');
          ParamByName('QTN').AsFloat:= cdsBalancoItensQTN.AsFloat;
      end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Salve ou Cancele o lançamento do balanço antes de sair.', 'Atenção',
  ...
  begin
    ...
    Finally
      ...
      begin
        ...
        begin
          Application.MessageBox('Este item já foi incluso no Balanço',
            'Atenção', MB_OK + MB_ICONEXCLAMATION);
          cdsBalancoItens.Cancel;
          ControlaBtnsItens;
          btnItensIncluir.SetFocus;
