...
var
  ...
  // Armazena a diferença que faltar no desconto após a divisão e o arredondamento
begin
  descDifeArred := 0;
  valorPago := 0;
  foraDesconto := 0;
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente cancelar o Pagamento?'), 'Atenção',
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Primeiro gere as parcelas a Pagar.', 'Atenção',
  ...
  if (modo = 'Pagamento') then
    ...
    Application.MessageBox('Inclusão de despesa realizada com sucesso!',
      'Inclusão de Despesa', MB_OK + MB_ICONEXCLAMATION);
