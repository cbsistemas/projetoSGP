...
begin
  if fdsOSSTATUS.AsString = 'FC' then
    Application.MessageBox('Você não pode altera uma OS já Fechada.',
...
begin
  ...
  begin
    Case MessageBox(Application.Handle,
      Pchar('Nenhum Produto foi lançado nessa OS, deseja Gravar Assim Mesmo?'),
  ...
  begin
    Application.MessageBox
      ('Confira se você lançou ao menos 1 técnico e 1 serviço nesta Ordem de Serviço!',
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir este serviço desta OS?'), 'Atenção',
...
begin
  Case MessageBox(Application.Handle,
    Pchar('Deseja realmente excluir este produto desta OS?'), 'Atenção',
    MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) of
    idYes:
      begin
