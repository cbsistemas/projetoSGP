...
begin
  ...
  begin
    ...
    // Retorna Valor de Devolução em Dinheiro
    Close;
    SQL.Clear;
    SQL.Add('SELECT SUM(VLR_VENDA) AS TOT FROM MOVIMENTO_CAIXA WHERE MOEDA = ' +
    ...
    // Retorna Valor Venda em Nota Promissória
    Close;
    SQL.Clear;
...
begin
  ...
  begin
    Application.MessageBox
      ('Primeiro informe um numero de CAIXA para a pesquisa.', 'Atenção',
...
begin
  ...
  begin
  Application.MessageBox('Não ha nenhuma movimentação de SANGRIA ou SUPRIMENTO nesse período', 'Atenção',MB_OK + MB_ICONEXCLAMATION);
  tbsTabela.Show;
  Abort;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox
        ('Número de caixa não registrado! Por fovor informe um número de caixa válido.',
...
begin
  ...
  begin
    Application.MessageBox('Primeiro informe um período para pesquisa.',
      'Atenção', MB_OK + MB_ICONEXCLAMATION);
    Abort;
  end;
