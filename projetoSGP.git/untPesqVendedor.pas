...
begin
  if (ActiveControl = JvDBGrid1) or (fdsVendedores.RecordCount = 1) then
  begin
    if Key = #13 then
    begin
      btnSelecionar.Click;
...
procedure TfrmPesqVendedor.JvDBGrid1DblClick(Sender: TObject);
begin
...
begin
  fdsVendedores.Close;
  fdsVendedores.SelectSQL.Clear;
  sql := 'SELECT ID, NOME, TELEFONE FROM VENDEDOR WHERE UPPER(NOME) LIKE ' +
